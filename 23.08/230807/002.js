...
// 방법 1
arr2.splice(1, 0, ...x)
arr2.splice(7, 0, ...x) // arr.push(...x) 또는 arr.push(1, 2, 3) 또는 arr.push(...[100, 200, 300])
arr2.push(100)

// 방법 2
arr2.splice(1, 0, x[0], x[1], x[2])
// 생략

////////////
// splice에서 인자값을 하나만 넣으면, 그 인덱스부터 끝까지 다 지운다.
// splice에서 인자값을 두개만 넣으면, 두번째 인자값부터 끝까지 다 지운다.
arr2.splice(1) // 1번째부터 끝까지 다 지워라
arr2.splice(1, 2) // 1번째부터 2개를 지워라(삽입값은 없습니다.)
...
arr2.slice(1, 3) // 1번째부터 3번째 전까지(2번째까지) 자른다.
arr2.slice(1, 10000) // index에 없는 값도 오류가 생기지 않습니다! splice도 마찬가지입니다.
arr2.slice(100, 10000) // index에 없는 값도 오류가 생기지 않습니다! splice도 마찬가지입니다.